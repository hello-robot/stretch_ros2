...
class JointTrajectoryAction(Node):
    ...
    def execute_cb(self, goal_handle):
        ...
        updates = [c.update(commanded_joint_names, self.error_callback,
        ...
        if num_valid_points <= 0:
            ...
            self.error_callback(goal_handle, FollowJointTrajectory.Result.INVALID_JOINTS, err_str)
            self.node.robot_mode_rwlock.release_read()
            return
        ...
        for pointi, point in enumerate(goal.trajectory.points):
            ...
            valid_goals = [c.set_goal(point, self.error_callback, self.node.fail_out_of_range_goal)
                           for c in self.command_groups]
            ...
            while not all(goals_reached):
                if (self.node.get_clock().now() - goal_start_time) > self.node.default_goal_timeout_duration:
                    ...
                    self.error_callback(goal_handle, FollowJointTrajectory.Result.PATH_TOLERANCE_VIOLATED, err_str)
                    self.node.robot_mode_rwlock.release_read()
                    return
        ...
        self.success_callback(goal_handle, "Achieved all target points.")
        self.node.robot_mode_rwlock.release_read()
        return
